...
> ⚠ mart_order_daily is pre-aggregated to store x day, so orders are summed from a count column rather than counted distinct. Counting DISTINCT order_id here would be a column that does not exist at this grain. orders_count is DELIVERED orders. 77,273 headers (5.0%) carry a basket the customer built and no fulfilled line - a total stockout at pick time - and they are excluded because this metric is defined per delivered order. Delivered aov is Rs 291.84; dividing by every header instead gives a lower figure, and mart_order_daily.placed_orders and .unfulfilled_orders carry that other denominator rather than hiding it. The rejected aov is measured as `aov_all_headers` by analytics/published.py rather than quoted here - it was the one figure in this note that could not be re-derived from a run without assuming which orders carry item rows.
