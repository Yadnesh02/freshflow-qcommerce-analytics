...
|---|---|---|---|---|
| **S5.0** ✅ | `warehouse.yml` — the full-year build in Actions: 365-day simulate → `dbt build` → the forecast chain → the five optimisers → demo slice → publish | 2h | Actions | **MET 2026-09-05 at `4348626`, 16m03s green end to end.** All seven anchors reproduced on a clean runner, so every figure in this document and the README is confirmed undamaged by the laptop's faulty RAM. Three runs to get there, and each failure was worth having: the first missed Sprint 3's forecast chain (`expiry-risk` needs `mart_demand_forecast`); the second was a *re-run* of the same commit, which never picks up a fix; the third exposed the slice-size fault below. **The anchors were then deliberately re-derived at `379997d`**: `04737af` gave each simulator component its own random substream, which changes every draw. The two structural anchors held exactly (14 stores, 23 elasticity cells); the rest moved 0.2–2.1%. **Every figure quoted in this document and the README predates that and is stale until re-derived.** |
| **S5.1** ✅ | Experiment harness + `experiment.yml`: 30 seeds × 90 days × 2 policies as a **job matrix**, one seed per runner, **common random numbers** | 3.5h | Actions | **MET 2026-09-05 at `14c693b`** — 30 jobs green in 7m13s. **The gate had to be restated: "same seed reproduces identical demand under both policies" is unachievable and would have passed vacuously**, since Policy B changes prices, demand is elastic, and the markdown optimiser currently recommends nothing — an inert arm produces exactly that equality. Replaced by three assertions: exogenous draws identical across arms; the arms nonetheless diverge; and one component's draws do not move when another consumes more. `04737af` made the first structural rather than tested. Pairing cuts the margin standard error by **72.0%**, which is CRN measurably working (re-derived at run 34030455838 from the clean-build bundle; it was 72.8% on the laptop-fitted one). **Read the 90-day arms with the horizon in mind:** they show write-offs *down* 21% where the 180-day holdout shows them roughly doubling, which is the short-horizon trap this project hit three times, not a contradiction. |
| **S5.2** ✅ | Store-level randomized holdout + DiD with parallel-trends check; restrict transfers within arms (SUTVA) | 2.5h | Actions | **MET at `8a5b5a7`** — 30 seeds, 180 days (45 pre / 135 post). **Parallel trends hold on all five metrics** (p 0.60–0.85). Margin **+5.66%** [CI +3.9, +7.4], units lost −14.88%, and **write-off +94.97%** — Policy B wastes nearly double. SUTVA is enforced as a filter: any transfer crossing the treatment boundary is dropped, so this measures a *partial rollout* while S5.1 measures full coverage. |
| **S5.3** ✅ | Sensitivity: elasticity ±30%, forecast error ×1.5, shelf life −1 day, **delivery cost ₹0–₹70/order**, **markdown disposal cost ₹0–₹50/unit** | 2h | Actions (matrix over the sweep) + `warehouse.yml` | **MET at `14f19b5`** — 9 settings × 10 seeds. **Margin survives every perturbation, +5.6% to +6.6%.** So does the write-off penalty, +15.6% to +89.0% — the sign never flips, though the floor is far lower than the laptop-fitted bundle suggested (+57%), and it is dispersion ×1.5 that pulls it down. **`units_expired` never clears p < 0.05, so "fewer units expired" is not a defensible claim.** Elasticity ±30% changes nothing (+88.7 vs +89.0 on write-off). Disposal cost ₹0→50 barely moves the write-off penalty (+88.5 → +88.1) while margin slides +6.4% → +5.6%, which is why S5.4 was needed. Delivery cost is an *analysis* sweep and runs in `warehouse.yml`. |
| **S5.4** ✅ | Ablation: B minus each component, attribute the gain | 1.5h | Actions (matrix over components) | **MET at `14f19b5`** — 5 components × 10 seeds. **Gate satisfied: residuals are −1.2% to +3.3% against totals up to 89.1%**, so the components are separable and per-component figures are quotable. **The newsvendor is the whole story: it contributes +89.1 of the +88.6% write-off penalty and −14.8 of the −13.4% stockout reduction.** Markdown restraint is the largest single margin contributor at **+4.0 of +6.4%** — unchanged to the decimal after the bundle was re-fitted, which is the strongest thing that can be said for it. Transfers contribute at most 0.3% on any metric. Re-derived at run 34031734404. |
| **S5.5** ✅ | `mart_experiment_readout` + Executive page readout. Results land as parquet under `data/experiment/`, read by a dbt source via `external_location` like the other thirteen. | 2h | Actions, then committed | **MET at `1ebd82b`.** Chain complete: workflow → readout parquet (committed) → dbt source → mart → `/experiment` → Executive page, SQL echoed at every step. **Four of six plan rows filled; the other two are named with reasons rather than fudged** — retention is customer-level while the holdout randomises stores, and WAPE has no Policy A column. **The table overturned the headline:** GM-AWM, the declared north star, falls **22.22% → 20.76%**, so Policy B is 1.35pp worse (re-derived at `15ee505` from a bundle a clean build fits; the laptop-fitted bundle gave 22.21% → 20.74% and −1.37pp, so the correction moved the headline by 0.02pp and changed nothing). Earlier "+6.4% margin" was a rupee *level*; the north star is a *rate net of wastage*. |
| **S5.6** ✅ | Dagster asset graph, daily schedule, asset checks | 2.5h | local venv | **MET at `8c337d1`** — **60 assets, 362 checks**, one lineage from the raw feeds through 54 individual dbt model assets to the decision tables. Grouped by warehouse layer (marts 22, staging 15, seed 3) rather than dumped in `default`. **Two checks, and the difference matters: `anchors_hold` is blocking** so a build whose figures moved stops before the recommendations regenerate; the figures report is not. Daily schedule at 05:30 IST, **excluding the simulator** — regenerating the world nightly would make every historical figure irreproducible. Screenshot in `docs/img/`. |
| **S5.7** ✅ | Streamlit page 6 (Data Quality) + Soda freshness checks | 1.5h | Codespace | **MET — the alert was made to fire.** `tests/test_soda.py` builds a coverage table where one feed stops three days early and requires the scan to go red; its twin requires the same scan to come back clean when every feed is current. Without the pair, a freshness check on a warehouse where nothing is stale is the seventh instance of this project's recurring bug. **Soda runs in its own `uv` environment and is deliberately absent from `pyproject.toml`**: soda-core caps protobuf below what dbt-core 1.11 requires, so a resolver satisfying both drags dbt-core to 1.8.8 or a 2.0 release candidate — downgrading the transformation layer to accommodate the thing that checks it. A test asserts it never enters the lockfile. **No wall-clock `freshness()` check**, because the simulated world ends on a fixed date and such a check would fail a little harder every day while meaning nothing; staleness is measured against the world's own last day. **Missing partitions warn and do not fail** — the clickstream outage is a documented defect that is not backfilled, and a step that goes red on every healthy build gets switched off. The workflow step is blocking on failures only; it is wired into `warehouse.yml` but has not yet had a run of its own. |
| **S5.8** ✅ | `sql_showcase/` — 15 documented queries | 2.5h | Codespace, verified in Actions | **MET.** Fifteen queries, executed by `tests/test_sql_showcase.py` rather than stored: each must run against a built warehouse and return rows, so a query that stops compiling when a column is renamed fails CI instead of waiting to be found in an interview — verified by renaming a column and watching it go red. Each file declares `question`, `technique` and **`needs_days`**, and the row assertion applies only where the warehouse is long enough; the declaration lives in the file, so there is no exemption list to forget. **Two things the data refuses:** year-over-year is impossible on a twelve-month world, so query 08 is month-over-month with the `LAG` that becomes YoY at offset 12; and the clickstream carries no `customer_id` and nothing between `pdp_view` and an order, so query 09 is browse → ordered → served and query 04 sessionises store-SKU attention rather than users. **Query 15 reconciles `received = sold + written off + remaining` to a residual of exactly zero across all 648,148 reconciled batches, and breaks on exactly the 6,509 the warehouse already flags** — the batches whose receipt was lost to the injected null-`batch_id` defect. Query 12 shows the SCD2 point-in-time join against the naive one: using today's cost for a year of sales misstates margin by up to 96% on the 321 SKUs that changed. |
| **S5.9** ✅ | `docs/business_case.pdf` (2 pages), OBS walkthrough video, ADRs, final README | 2.5h | anywhere | **Built, not yet gated.** [`docs/business_case.md`](business_case.md) — two pages, exec tone, leading with the negative result and what we would ship anyway. **Markdown rather than PDF deliberately:** the source is diffable in a pull request, which is the same argument this project makes for a metric registry over a `.pbix`, and adding a rendering dependency for one document is a cost `packages.yml` refuses elsewhere. Eight [ADRs](adr/), each carrying the alternative that was seriously considered — including the two that were rejected on evidence rather than taste (Git LFS for the demo slice, Soda as a project dependency). Résumé bullets and the 60-second story in [`docs/resume_and_story.md`](resume_and_story.md), with the three framings to avoid. README updated: badges, six pages, and the experiment result, which was **not in it at all**. **MET.** §14 stands at 11 ticked, 2 deferred by decision, 0 open. The clean-clone line closed on warehouse run 34028321712, which ran the whole chain including `recommend` from a clone on a clean runner; the walkthrough recording and the 60-second rehearsal were deferred deliberately on 2026-09-06, with the script and the story written so picking either up costs no thinking. |
...
| Gate | When | Test |
|---|---|---|
| **G1** ✅ | End Sprint 1 | On-hand never negative; FEFO strictly oldest-first; every injected defect countable — **PASSED.** `tests/test_gate.py` proves the verifier itself works by corrupting a raw layer fifteen ways and requiring the matching check to go red: negative stock, selling expired stock, cross-store fulfilment, an orphaned batch reference, out-of-order FEFO, a missing partition. `tests/test_fefo.py` carries the three invariants directly, and `test_documented_defects_are_explained_not_failed` with `test_damage_beyond_the_documented_amount_still_fails` is the countability clause in both directions. |
| **G2** ✅ | End Sprint 2 | `agg_store_sku_day` revenue ties to raw order totals **exactly**; demo slice < 80 MB — **PASSED.** `transform/tests/assert_agg_revenue_ties_to_raw_order_totals.sql` states the clause as the plan words it and runs on every `dbt build`, so the tie is re-asserted on every commit rather than checked once. The size half is `test_manifest_stays_under_the_deployment_gate`, which measures the **published** asset rather than the local build — the two can differ, and only one of them is what the app downloads. |
...
| **G4** ✅ | End Sprint 4 | Markdown depth increases monotonically as DTE falls, holding the demand **rate** constant — **and the sweep must move**: run at an inelastic coefficient every depth is zero and "non-decreasing" passes while testing nothing. `test_an_inelastic_coefficient_makes_the_g4_sweep_vacuous` holds that shut. **PASSED:** all three clauses are separate tests - `test_depth_rises_as_expiry_approaches` for monotonicity, `test_the_g4_sweep_actually_moves` for the sharpened clause, and the vacuity test above, which asserts the sweep is *still* flat at an inelastic coefficient so the fixture cannot silently start passing for the wrong reason. |
| **G5** ✅ | End Sprint 5 | Experiment reproducible from a seed; every resume number traceable to `mart_experiment_readout`. **Sharpened 2026-09-05:** reproducible *in CI* — a workflow anyone can re-run from a clone, not a run that happened once on one machine. The hardware fault that forced this is the best argument for it. **Not yet marked, and deliberately so — Sprint 5 is not finished.** The reproducibility half is evidenced: `experiment.yml` and `holdout.yml` run 30 seeds each from a seed on a clean runner, `tests/test_experiment.py` asserts common random numbers hold *and* that the arms still diverge so the gate cannot pass on an inert policy, and the readout parquet is committed so the mart rebuilds from a clone. The traceability half had a hole until `b9a6df2`: **`mart_experiment_readout` was the only mart in the project with no schema entry** — the terminal table of the chain every published number is supposed to trace to was the one table nothing asserted. It now carries a contract, including a singular test that a row either has its measurement or the stated reason it cannot. **PASSED at `45b1251`** — those tests ran in `dbt build` on a clean runner and came back green. **Then run 34028321712 found the gap this gate exists to catch, and it was in the input rather than the output.** The committed `policy_bundle.json` — the file Policy B actually reads — had been exported from the laptop's stale warehouse: all 23 elasticity cells differed materially from what a clean build fits, several with the sign reversed, and it carried 14 identified cells against CI's 13. So the experiment reproduced from a *clone*, which is what the gate says, while not reproducing from a *rebuild*, which is what it means. The CI-fitted bundle is committed and the four workflows that read it are being re-run. **The bundle's `provenance` block recorded cell counts and not the build that fitted them, which is why this was invisible. Fixed: it now carries `fitted_from` - the two anchors that identify a build, plus the workflow run and commit - and `tests/test_policy_bundle_provenance.py` compares that against the pinned anchors with no warehouse in front of it. Pointed at the laptop's own figures the guard goes red, which is the check that was missing.** |
