...
| **S5.3** ✅ | Sensitivity: elasticity ±30%, forecast error ×1.5, shelf life −1 day, **delivery cost ₹0–₹70/order**, **markdown disposal cost ₹0–₹50/unit** | 2h | Actions (matrix over the sweep) + `warehouse.yml` | **MET at `14f19b5`** — 9 settings × 10 seeds. **Margin survives every perturbation, +5.7% to +6.6%.** So does the write-off penalty, +57% to +92% — it is not an artefact of any single assumption. **`units_expired` never clears p < 0.05, so "fewer units expired" is not a defensible claim.** Elasticity ±30% changes nothing (+92.0 vs +91.5). Disposal cost barely moves anything, which is why S5.4 was needed. Delivery cost is an *analysis* sweep and runs in `warehouse.yml`. |
| **S5.4** ✅ | Ablation: B minus each component, attribute the gain | 1.5h | Actions (matrix over components) | **MET at `14f19b5`** — 5 components × 10 seeds. **Gate satisfied: residuals are −1.2% to +3.0% against totals up to 91.1%**, so the components are separable and per-component figures are quotable. **The newsvendor is the whole story: it contributes +92.3 of the +91.1% write-off penalty and −14.4 of the −12.9% stockout reduction.** Markdown restraint is the largest single margin contributor (+4.0 of +6.4%). Transfers contribute 0.0% on every metric. |
